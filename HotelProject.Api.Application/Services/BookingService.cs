...
namespace HotelProject.Api.Application.Services;
...
public class BookingService(HotelProjectDbContext context, IUsersService usersService, IMapper mapper) : IBookingService
{
    public async Task<Result<IEnumerable<GetBookingDto>>> GetBookingsForHotelAsync(int hotelId)
    {
        ...
        // Retrieve bookings for the hotel and project to GetBookingDto
        var bookings = await context.Bookings
            .Where(b => b.HotelId == hotelId)//get all bookings for the hotel that matches the hotelId coming from the parameter
            .OrderBy(b => b.CheckIn)
            .ProjectTo<GetBookingDto>(mapper.ConfigurationProvider)//project the bookings to GetBookingDto using AutoMapper
            .ToListAsync();
    ...
    // Function to check if the booking dates overlap with existing bookings for the same hotel and user
    // It checks if there are any bookings for the same hotel and user where the check-in and check-out dates overlap with the new booking dates
    // If a bookingId is provided, it excludes that booking from the overlap check (useful for updating an existing booking)
